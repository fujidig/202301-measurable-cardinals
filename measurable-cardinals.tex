...
\begin{document}
	...
	\begin{defi}
		$S$を無限集合とする． $S$上の(一様かつ$\sigma$加法的な確率)\textbf{測度} (measure)とは$\mu \colon \Pow(S) \to [0, 1]$であって，次を満たすものである：
		\begin{enumerate}
	...
	\begin{defi}
		$\mu$を$S$上の測度とする．$A \subset S$が\textbf{原子} (atom)であるとは，$\mu(A) > 0$かつ任意の$X \subset A$に対して$\mu(X) = 0$または$\mu(X) = \mu(A)$となるものである．
		原子が存在しない測度を原子なし (atomless)の測度という．
	\end{defi}
	
	\begin{defi}
		\begin{enumerate}
			\item 基数$\kappa$が\textbf{可測基数} (measurable cardinal)であるとは，$\kappa$上の$\kappa$完備な非単項超フィルターが存在することを言う．
			\item 基数$\kappa$が\textbf{実数値可測基数} (real valued measurable cardinal)であるとは，$\kappa$上の$\kappa$加法的測度が存在することを言う．
		\end{enumerate}
	\end{defi}
	...
	\begin{defi}
		集合$S$上のイデアル$I$で\textbf{$\sigma$飽和的} ($\sigma$-saturated)であるとは，$I$に属さない$S$の部分集合族で互いに素なものはどれも，族の濃度が可算であることを意味する．
	\end{defi}
	...
	$\kappa$の部分集合の列$(X_\alpha : \alpha < \kappa)$についてその\textbf{対角共通部分} (diagonal intersection)$\diagintr_{\alpha < \kappa} X_\alpha$というのは
	\[
	...
	\]
	を\textbf{対角和集合} (diagonal union)という．
	
	フィルターが\textbf{正規} (normal)であるとは，それが対角共通部分を取る操作で閉じていることであった．
	イデアルが正規というのは双対フィルターが正規であること，すなわち対角和を取る操作で閉じていることを意味する．
	$f \colon X \to \kappa$ ($X \subset \kappa$)が\textbf{押し下げ関数} (regressive function)であるとは，任意の$\alpha \in X$について$f(\alpha) < \alpha$となることであった．
	...
	構造$\Ult = (\Ult, \in^*)$を考える．これを宇宙$V$の\textbf{超冪} (ultrapower)という．通常のモデル理論におけるŁośの定理は宇宙の超冪でも成り立つことが確認できる：
	\[
	...
	\]
	を得る．つまり，モデル理論で使っていた用語を拝借すると，$j \colon V \to \Ult$は\textbf{初等埋め込み} (elementary embedding)である．
	...
	\begin{defi}
		...
		\]
		とおき，$j$の\textbf{臨界点} (critical point)と呼ぶ．
	\end{defi}
	...
	以下の補題で\textbf{$M$超フィルター} ($M$-ultrafilter)というのは次を満たす$D \subset \Pow^M(\kappa)$である：
	\begin{enumerate}
		\item $\emptyset \not \in D, \kappa \in D$.
	...
	$V[G]$で超冪$\Ult_G(V)$を考える．これを\textbf{ジェネリック超冪} (generic ultrapower)という．
	...
	$\kappa$完備イデアル$I$が\textbf{峻厳} (precipitous)であるとは，そのジェネリック超冪$\Ult_G(V)$が常にwell-foundedとなることを意味する．
	...
	\begin{proof}
		...
		最後に$T \in I^+$を示そう．$I$の定義より，どんな$Z \in D$についても， $T \cap Z \ne \emptyset$であることを示せばよい．
